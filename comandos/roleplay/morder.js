module.exports = async (client, message, args) => {
    ...
    if(mencion === message.author) return message.reply('No te puedes morder a tí mism@. Bueno si, pero aquí no. :(')
    let txt = args.slice(1).join(' ')
    message.delete();
    const img = await gif('bite');
    const em = new Discord.MessageEmbed()
      .setTitle(`${message.author.username} mordió a ${mencion.username}. OwO`)
      .setDescription(txt)
      .setColor('RANDOM')
      .setImage(img)
  ...
  const log = new Discord.MessageEmbed()
    ...
    .setDescription('Se usó el comando `morder`')
