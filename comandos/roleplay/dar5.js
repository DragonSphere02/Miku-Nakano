module.exports = async (client, message, args) => {
  ...
  if(mencion === message.author) return message.reply('¿Te vas a chocarte los 5 a ti mism@?\nVeo que estás muy sól@. :(')

  let txt = args.slice(1).join(' ')
  message.delete();
  const img = await gif('highfive');
  const em = new Discord.MessageEmbed()
  .setTitle(`${message.author.username} chocó los 5 con ${mencion.username}.\n(〃￣︶￣)人(￣︶￣〃)`)
  .setDescription(txt)
  .setColor('RANDOM')
  .setImage(img)
  ...
  .setDescription('Se usó el comando `dar5`\n**Texto: **'+ txt)
