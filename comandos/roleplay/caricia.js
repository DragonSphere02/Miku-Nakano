module.exports = async (client, message, args) => {
  ...
  if(mencion === message.author) return message.reply('¿Te vas a acariciar a ti mism@?\nVeo que estás muy sól@. :(')

  let txt = args.slice(1).join(' ')
  message.delete();
  const img = await gif('cuddle');
  const em = new Discord.MessageEmbed()
  .setTitle(`${message.author.username} acarició a ${mencion.username}. ( ´･･)ﾉ(._.)`)
  .setDescription(txt)
  .setColor('RANDOM')
  .setImage(img)
  ...
  .setDescription('Se usó el comando `caricia`\n**Texto: **'+ txt)
