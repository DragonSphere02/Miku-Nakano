module.exports = async (client, message, args) => {
  ...
  if(mencion === message.author) return message.reply('¿Te vas a animar a ti mism@?\nVeo que estás muy sól@. :(')

  let txt = args.slice(1).join(' ')
  message.delete();
  const img = await gif('cheer');
  const em = new Discord.MessageEmbed()
  .setTitle(`${message.author.username} animó a ${mencion.username}. (*≧︶≦))(￣▽￣* )ゞ`)
  .setDescription(txt)
  .setColor('RANDOM')
  .setImage(img)
  ...
  .setDescription('Se usó el comando `animar`\n**Texto: **'+ txt)
