module.exports = async (client, message, args) => {
    ...
    if(mencion === message.author) return message.reply('No te puedes perseguir a tí mism@. :)')
    let txt = args.slice(1).join(' ')
    message.delete();
    const img = await gif('chase');
    const em = new Discord.MessageEmbed()
      .setTitle(`${message.author.username} está persiguiendo a ${mencion.username}. Σ(っ °Д °;)っ`)
      .setDescription(txt)
      .setColor('RANDOM')
      .setImage(img)
  ...
  const log = new Discord.MessageEmbed()
    ...
    .setDescription('Se usó el comando `perseguir`')
