module.exports = async (client, message, args) => {
    ...
    if(mencion === message.author) return message.reply('No te puedes molestar a ti mism@. xD')
    let txt = args.slice(1).join(' ')
    message.delete();
    const img = await gif('bully');
    const em = new Discord.MessageEmbed()
      .setTitle(`${message.author.username} está molestando a ${mencion.username}. ಠ﹏ಠ`)
      .setDescription(txt)
      .setColor('RANDOM')
      .setImage(img)
  ...
  const log = new Discord.MessageEmbed()
    ...
    .setDescription('Se usó el comando `molestar`')
