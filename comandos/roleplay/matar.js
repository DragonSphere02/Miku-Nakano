module.exports = async (client, message, args) => {
    ...
    if(mencion === message.author) return message.reply('No hagas la suicidación. :(')
    let txt = args.slice(1).join(' ')
    message.delete();
    const img = await gif('kill');
    const em = new Discord.MessageEmbed()
      .setTitle(`${message.author.username} mató a ${mencion.username}. (╯‵□′)╯︵┻━┻`)
      .setDescription(txt)
      .setColor('RANDOM')
      .setImage(img)
  ...
  const log = new Discord.MessageEmbed()
    ...
    .setDescription('Se usó el comando `matar`')
