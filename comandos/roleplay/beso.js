module.exports = async (client, message, args) => {
  ...
  if(mencion === message.author) return message.reply('¿Te vas a besar a ti mism@?\nVeo que estás muy sól@. :(')

  let txt = args.slice(1).join(' ')
  message.delete();
  const img = await gif('kiss');
  const em = new Discord.MessageEmbed()
  .setTitle(`${message.author.username} besó a ${mencion.username}. (* ￣3)(ε￣ *)`)
  .setDescription(txt)
  .setColor('RANDOM')
  .setImage(img)
  ...
  .setDescription('Se usó el comando `beso`\n**Texto: **'+ txt)
