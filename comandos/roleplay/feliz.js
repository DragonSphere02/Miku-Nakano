module.exports = async (client, message, args) => {
  ...
  if(mencion === message.author) return message.reply('Sé que estás felíz contigo mism@, pero no puedes hacer esto. :(')
  if (!mencion) {
    let txt = args.join(' ')
    message.delete()
    const img = await gif('happy');
    ...
    const img = await gif('happy');
    const em = new Discord.MessageEmbed()
    .setTitle(`${message.author.username} está feliz con ${mencion.username}. (✿◠‿◠)`)
    .setDescription(txt)
    .setColor('RANDOM')
    .setImage(img)
    .setFooter(`Solicitado por ${message.author.username}`, message.author.avatarURL({dynamic: true}))
  ...
  .setDescription('Se usó el comando `feliz`')
