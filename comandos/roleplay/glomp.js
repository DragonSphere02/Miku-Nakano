module.exports = async (client, message, args) => {
    ...
    if(mencion === message.author) return message.reply('No puedes abrazarte a ti mism@. :(')
    let txt = args.slice(1).join(' ')
    message.delete();
    const img = await gif('glomp');
    const em = new Discord.MessageEmbed()
      .setTitle(`${message.author.username} se abalanzó hacia ${mencion.username}. ~(=^‥^)ノ`)
      .setDescription(txt)
      .setColor('RANDOM')
      .setImage(img)
  ...
  const log = new Discord.MessageEmbed()
    ...
    .setDescription('Se usó el comando `glomp`')
